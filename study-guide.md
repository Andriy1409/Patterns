# Гайд з підготовки — Патерни + Мережі + Бази даних + Алгоритми

> **Мета:** єдина точка входу по всьому матеріалу репозиторію.  
> **Контекст:** підготовка до технічної співбесіди в компанії, що спеціалізується на **мережевому моніторингу та управлінні (Network Management)** — SolarWinds — плюс повноцінні серії з баз даних та з алгоритмів і структур даних до сіньйорського рівня.  
> **Загальний обсяг:** 23 патерни проектування (GoF, C#) + 10 детальних розборів мережевих технологій + 16 документів з баз даних (реляційні + NoSQL) + 18 документів з алгоритмів та структур даних ≈ 62 000+ рядків матеріалу.
...
5. [Частина III — Бази даних (16)](#частина-iii--бази-даних-16)
   ...
   - [NoSQL — 7](#nosql--7)
6. [Частина IV — Алгоритми та структури даних (18)](#частина-iv--алгоритми-та-структури-даних-18)
   - [Структури даних — 8](#структури-даних--8)
   - [Алгоритми — 9](#алгоритми--9)
7. [Рекомендований план підготовки](#рекомендований-план-підготовки)
8. [Наскрізні теми, що з'являються в кількох файлах](#наскрізні-теми-що-зявляються-в-кількох-файлах)
9. [Швидкий чекліст перед співбесідою](#швидкий-чекліст-перед-співбесідою)
...
АлгоритмиТаСтруктуриДаних/                (Junior → Senior)
├── 0-complexity-analysis-big-o.md        ← прочитати першим, до обох підпапок
│
├── СтруктуриДаних/
│   ├── 1-arrays-and-linked-lists.md
│   ├── 2-stacks-and-queues.md
│   ├── 3-hash-tables.md
│   ├── 4-trees-and-bst.md
│   ├── 5-heaps-and-priority-queues.md
│   ├── 6-tries.md
│   ├── 7-graph-representations.md
│   └── 8-union-find-disjoint-set.md
│
└── Алгоритми/
    ├── 1-sorting-algorithms.md
    ├── 2-binary-search-and-array-techniques.md
    ├── 3-recursion-and-backtracking.md
    ├── 4-divide-and-conquer.md
    ├── 5-greedy-algorithms.md
    ├── 6-dynamic-programming.md
    ├── 7-graph-traversal-bfs-dfs.md
    ├── 8-shortest-paths-and-mst.md
    └── 9-np-completeness-and-interview-strategy.md  ← капстоун усієї серії алгоритмів
```
...
## Частина IV — Алгоритми та структури даних (18)

Ця серія написана до **сіньйорського рівня** й побудована так, що файли **посилаються один на одного**: пізніші файли явно повертаються до прикладів і "клiфхенгерів" з попередніх (наприклад, невдалий жадібний розв'язок Knapsack з `5-greedy-algorithms.md` отримує коректне рішення через DP у `6-dynamic-programming.md`). Тому читати варто саме в номерному порядку, а не вибірково.

**Прочитати першим, до обох підпапок:** `0-complexity-analysis-big-o.md` — Big O/Θ/Ω, амортизована складність, чому "працювало на деві" не гарантує роботу в продакшені.

### Структури даних — 8

| # | Тема | Файл | Ключова сіньйорська ідея |
|---|---|---|---|
| 1 | Масиви, динамічні масиви, зв'язні списки | `1-arrays-and-linked-lists.md` | Cache locality часто важливіша за теоретичну Big O на практиці |
| 2 | Стек та черга (+ Deque) | `2-stacks-and-queues.md` | Кільцевий буфер, зв'язок рекурсії зі стеком викликів, Undo/Redo двома стеками |
| 3 | Хеш-таблиці | `3-hash-tables.md` | Контракт Equals/GetHashCode; зв'язок з Redis і consistent hashing з БД-серії |
| 4 | Дерева та BST | `4-trees-and-bst.md` | Виродження BST на відсортованих даних; звідки в БД беруться B-дерева |
| 5 | Купа та Priority Queue | `5-heaps-and-priority-queues.md` | O(n) heapify; фундамент для Дейкстри/Прима далі в серії |
| 6 | Trie (префіксне дерево) | `6-tries.md` | Автодоповнення за O(L), незалежно від кількості слів |
| 7 | Представлення графів | `7-graph-representations.md` | Adjacency list vs matrix — вибір за щільністю графа, не "за замовчуванням" |
| 8 | Union-Find (DSU) | `8-union-find-disjoint-set.md` | Union by rank + path compression → майже O(1); основа алгоритму Крускала |

### Алгоритми — 9

| # | Тема | Файл | Ключова сіньйорська ідея |
|---|---|---|---|
| 1 | Сортування | `1-sorting-algorithms.md` | Стабільність сортування; чому O(n log n) — нижня межа для порівняльних сортувань |
| 2 | Бінарний пошук та техніки масивів | `2-binary-search-and-array-techniques.md` | Класичні off-by-one пастки; two pointers vs хеш-таблиця; sliding window |
| 3 | Рекурсія та бектрекінг | `3-recursion-and-backtracking.md` | Наївний Фібоначчі O(2ⁿ) як клiфхенгер до DP; шаблон вибір-дослідження-скасування |
| 4 | Розділяй і володарюй | `4-divide-and-conquer.md` | Теорема майстра застосована "вручну" до нових рекурентностей |
| 5 | Жадібні алгоритми | `5-greedy-algorithms.md` | Fractional vs 0/1 Knapsack — де жадібність працює, а де провально ні (клiфхенгер) |
| 6 | Динамічне програмування | `6-dynamic-programming.md` | Розв'язує обидва клiфхенгери файлів 3 і 5 цієї серії |
| 7 | Обхід графів: BFS та DFS | `7-graph-traversal-bfs-dfs.md` | Виявлення циклу (directed vs undirected), топологічне сортування |
| 8 | Найкоротші шляхи та MST | `8-shortest-paths-and-mst.md` | Закриває три форвард-посилання (heaps, Union-Find, greedy) одночасно |
| 9 | NP-повнота + стратегія співбесіди (капстоун) | `9-np-completeness-and-interview-strategy.md` | Повний decision-framework "яку техніку застосувати" по всій серії |

> 💡 Файл `9-np-completeness-and-interview-strategy.md` — не просто ще одна тема, а **підсумковий синтез усієї серії**: явний decision-tree "яка ознака задачі → яка техніка з якого файлу", плюс покроковий процес розв'язання незнайомої задачі на співбесіді.
...
### Крок 7 — Алгоритми та структури даних (окремий трек; файли посилаються один на одного за номерами)
23. `0-complexity-analysis-big-o.md` — обов'язковий вхід, без нього незрозумілі всі складнісні оцінки далі
24. `СтруктуриДаних/1` → `2` → `3` → `4` — масиви/списки, стек/черга, хеш-таблиці, дерева/BST (базові структури)
25. `СтруктуриДаних/5` → `6` → `7` → `8` — купи, Trie, представлення графів, Union-Find (просунуті структури)
26. `Алгоритми/1` → `2` → `3` — сортування, бінарний пошук/техніки масивів, рекурсія/бектрекінг
27. `Алгоритми/4` → `5` — розділяй-і-володарюй, жадібні алгоритми (**зверни увагу на клiфхенгер 0/1 Knapsack**)
28. `Алгоритми/6` — динамічне програмування — розв'язує клiфхенгери файлів 3 і 5 одразу
29. `Алгоритми/7` → `8` — обхід графів (BFS/DFS), найкоротші шляхи та MST
30. `Алгоритми/9` — читати останнім: капстоун з повним decision-framework для незнайомих задач на співбесіді
...
- **B-дерево в базах даних — це узагальнення BST** (`4-trees-and-bst.md` з серії алгоритмів ↔ `3-indexes-and-query-optimization.md` з серії БД) — той самий принцип "поділи навпіл", але оптимізований під сторінки диску замість порівнянь у пам'яті.
- **Consistent hashing з'являється двічі** — в `7-graph-representations.md`/`3-hash-tables.md` (структури даних) як загальний принцип, і в `4-wide-column-stores-cassandra.md` (Cassandra) як конкретне застосування до розподілу даних по вузлах кластера.
- **Saga = Union-Find/Кластеризація мислення, тільки для мікросервісів** — `9-distributed-transactions-and-saga.md` (БД) і граф-алгоритми файли `7-graph-traversal-bfs-dfs.md`/`8-shortest-paths-and-mst.md` (алгоритми) обидва моделюють "залежності між компонентами як граф" — топологічне сортування напряму застосовне до порядку деплою мікросервісів.
...
- [ ] Пояснити амортизовану складність append у динамічному масиві (чому O(1), а не O(n))
- [ ] Написати бінарний пошук без класичної помилки переповнення `(low + high) / 2`
- [ ] Чим Fractional Knapsack (жадібно розв'язується) відрізняється від 0/1 Knapsack (потребує DP)
- [ ] BFS для найкоротшого шляху в незваженому графі, Дейкстра — у зваженому (і чому Дейкстра ламається на від'ємних вагах)
- [ ] Контракт Equals/GetHashCode для власного класу як ключа Dictionary
- [ ] Union by rank + path compression — чому Union-Find майже O(1)
- [ ] Розпізнати "це схоже на NP-повну задачу" і перейти до наближеного/евристичного рішення, а не шукати точний поліноміальний алгоритм
